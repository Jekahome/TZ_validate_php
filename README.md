...
```
```
В тестовом не реализованы следующие требования:
- "обеспечения возможности масштабирования системы" ни на одном из направлений (были попытки для валиадторов)
- "Реализация должна соответствовать принципам ООП" есть проблемы с понимаем инкапсуляции и полиформизмаОбщие проблемы:
- Процедурная реализация, со всеми вытекающими последствиями
- Есть проблемы с процедурной реализацией и оптимизацией языковых конструкций
- Непрозрачные сигнатуры методов классов
- Использование колбеков без особой на то надобности
```
